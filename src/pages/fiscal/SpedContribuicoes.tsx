...
import { gerarArquivoSpedContribuicoes, downloadSpedContribuicoes, SpedContribConfig, BLOCOS_CONTRIBUICOES } from "@/lib/sped/contribuicoesGenerator";
...
const defaultLayouts: LayoutVersion[] = [
  { id: "1", codigo: "006", descricao: "Layout 006 - Vigente desde 01/2024", vigenciaInicio: "2024-01", vigenciaFim: "", ativo: true },
  { id: "2", codigo: "005", descricao: "Layout 005 - Vigente 01/2019 a 12/2023", vigenciaInicio: "2019-01", vigenciaFim: "2023-12", ativo: false },
];
...
function getColorForLine(line: string): string {
  ...
  const map: Record<string, string> = {
    ...
    "F": "text-violet-600 dark:text-violet-400",
...
export default function SpedContribuicoes() {
  const [periodoInicio, setPeriodoInicio] = useState("");
  const [periodoFim, setPeriodoFim] = useState("");
  const [tipoEscrituracao, setTipoEscrituracao] = useState("0");
  const [situacaoEspecial, setSituacaoEspecial] = useState("00");
  ...
  const [layouts, setLayouts] = useState<LayoutVersion[]>(() => {
    try {
      const stored = localStorage.getItem("sped-contribuicoes-layouts");
  ...
  const [blocosAtivos, setBlocosAtivos] = useState<Record<string, boolean>>(() =>
    Object.fromEntries(BLOCOS_CONTRIBUICOES.map(b => [b.id, true]))
  );
  ...
  const salvarLayouts = (updated: LayoutVersion[]) => {
    setLayouts(updated);
    localStorage.setItem("sped-contribuicoes-layouts", JSON.stringify(updated));
  };
  ...
  const getEmpresa = () => {
    try {
      ...
      if (stored) {
        ...
        if (sel) {
          return {
            cnpj: sel.cnpj || "", razaoSocial: sel.razaoSocial || "",
            fantasia: sel.nomeFantasia || "", inscricaoEstadual: sel.inscricaoEstadual || "",
            cep: sel.cep || "", logradouro: sel.endereco || "", numero: "0",
            complemento: "", bairro: sel.bairro || "",
            codigoCidade: sel.codigoCidade || "3550308", cidade: sel.cidade || "",
            uf: sel.uf || "", telefone: sel.telefone || "", email: sel.email || "",
    ...
    return {
      cnpj: "00000000000000", razaoSocial: "EMPRESA NAO CADASTRADA",
      fantasia: "EMPRESA", inscricaoEstadual: "ISENTO", cep: "00000000",
      logradouro: "", numero: "0", complemento: "", bairro: "",
      codigoCidade: "3550308", cidade: "SAO PAULO", uf: "SP",
      telefone: "0000000000", email: "", regime: "3", cnae: "4712100",
    };
  };
  ...
  const gerarArquivo = () => {
    ...
    const config: SpedContribConfig = {
      empresa: getEmpresa(),
      periodoInicio,
      periodoFim,
      tipoEscrituracao,
    ...
    const conteudo = gerarArquivoSpedContribuicoes(config);
    setConteudoArquivo(conteudo);

    const result = validarArquivoSped(conteudo, "contribuicoes");
    setValidacao(result);
    ...
    if (result.totalErros > 0) {
      ...
      toast.success("Arquivo SPED Contribuições gerado sem inconsistências!");
    }
  };

  const baixarArquivo = () => {
    if (!conteudoArquivo) return;
    downloadSpedContribuicoes(conteudoArquivo, periodoInicio);
  ...
  const revalidar = () => {
    ...
    const result = validarArquivoSped(texto, "contribuicoes");
    setValidacao(result);
    setModoEdicao(false);
  ...
  return (
    <div className="page-container">
      <PageHeader title="SPED Contribuições (EFD PIS/Cofins)" description="Geração do arquivo SPED Contribuições com controle de versão de layout" />
      ...
      <Tabs defaultValue="gerar">
        ...
        <TabsContent value="gerar" className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>Gerar SPED Contribuições</CardTitle>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select value={tipoEscrituracao} onValueChange={setTipoEscrituracao}>
                ...
                <div className="space-y-2">
                  ...
                  <Select value={situacaoEspecial} onValueChange={setSituacaoEspecial}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="00">00 - Normal</SelectItem>
                      <SelectItem value="01">01 - Extinção</SelectItem>
                      <SelectItem value="02">02 - Fusão</SelectItem>
                      <SelectItem value="03">03 - Incorporação/Incorporada</SelectItem>
                      <SelectItem value="04">04 - Incorporação/Incorporadora</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 p-3 border border-border rounded-lg bg-muted/30">
                  {BLOCOS_CONTRIBUICOES.map(bloco => (
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-base">Prévia do Arquivo SPED Contribuições</CardTitle>
                    ...
                    {Object.entries(contaBlocos()).sort().map(([bloco, count]) => (
                      <span key={bloco} className="flex items-center gap-1">
                        <span className={`w-3 h-3 rounded ${
                          ...
                          bloco === "D" ? "bg-cyan-600" :
                          bloco === "F" ? "bg-violet-600" :
                          bloco === "M" ? "bg-amber-600" :
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        exportarRelatorioValidacaoTxt(validacao, "contribuicoes", empresa.razaoSocial, `${periodoInicio} a ${periodoFim}`);
        ...
        <TabsContent value="layouts">
          <Card>
            <CardHeader>
              <CardTitle>Gerenciar Layouts do SPED Contribuições</CardTitle>
              <CardDescription>Atualize conforme publicações do Guia Prático da EFD-Contribuições</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-5 gap-3 items-end p-4 border border-border rounded-lg bg-muted/30">
                <div className="space-y-1">
                  ...
                  <Input placeholder="Ex: 007" value={novoLayout.codigo} onChange={e => setNovoLayout(p => ({ ...p, codigo: e.target.value }))} />
                </div>
                <div className="md:col-span-2 space-y-1">
                  <Label className="text-xs">Descrição</Label>
                  <Input placeholder="Layout 007 - Vigente desde..." value={novoLayout.descricao} onChange={e => setNovoLayout(p => ({ ...p, descricao: e.target.value }))} />
                </div>
                <div className="space-y-1">
