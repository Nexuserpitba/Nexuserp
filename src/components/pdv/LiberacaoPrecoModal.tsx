...
export function LiberacaoPrecoModal({ open, onClose, itemDescricao, precoAnterior, novoPreco, onAutorizado }: Props) {
  ...
  const handleAutorizar = async () => {
    ...
    try {
      ...
      insertLiberacao({ operador: "PDV", cliente: itemDescricao, clienteDoc: "", valorAutorizado: novoPreco, limiteDisponivel: precoAnterior, excedente: 0, motivo: `Alteração de preço de R$ ${precoAnterior.toFixed(2)} para R$ ${novoPreco.toFixed(2)} autorizada por ${data.user.nome}` });
      toast.success(`Alteração de preço autorizada por ${data.user.nome}`);
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-destructive">
            <ShieldAlert size={20} />
            Liberação para Alteração de Preço
          </DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div className="bg-muted/50 rounded-lg p-3 space-y-1">
            ...
            <div className="flex justify-between text-xs mt-2">
              ...
              <span className="font-bold">R$ {precoAnterior.toFixed(2)}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-muted-foreground">Novo preço:</span>
              <span className="font-bold text-destructive">R$ {novoPreco.toFixed(2)}</span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Input ref={inputRef} value={login} onChange={e => { setLogin(e.target.value); setErro(""); }} placeholder="E-mail" onKeyDown={e => e.key === "Enter" && document.getElementById("lib-preco-senha")?.focus()} />
          </div>
          <div className="space-y-2">
            <Label>Senha</Label>
            <div className="flex gap-2">
              <Input id="lib-preco-senha" type={showPassword ? "text" : "password"} value={senha} onChange={e => { setSenha(e.target.value); setErro(""); }} placeholder="Senha" onKeyDown={e => e.key === "Enter" && handleAutorizar()} />
